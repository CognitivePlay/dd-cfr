"""Tests the ``card_collection`` module."""
...
class TestDeck:
    ...
    def test_get_maximum_number_of_cards(self):
        assert card_collection.Deck.get_maximum_number_of_cards() == len(
            card.Suit
        ) * len(card.Value)
    ...
    def test_get_turn_up_card(self):
        deck = card_collection.Deck()
    ...
    def test_get_list_of_all_cards(self):
        expected = []
...
class TestHand:
    ...
    def test_get_card(self):
        ...
        for _ in range(5):
            hand.play(0)
            for i in range(-1, 7):
                if 0 <= i < hand.get_number_of_cards():
                    continue
